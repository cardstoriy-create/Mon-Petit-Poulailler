# Script : Update-Title-2026.ps1
# Mission : Mettre à jour le titre de l'article Réglementation pour 2026
...
# Sécurité : Création du dossier si inexistant (ne devrait pas arriver ici)
...
En France, la règle générale est la liberté : vous avez le droit d'élever des poules pour votre consommation personnelle (on parle alors d'élevage "d'agrément"). Cependant, la taille de votre poulailler détermine vos obligations administratives.

### Les 3 seuils de construction
Le Code de l'Urbanisme définit vos démarches selon l'emprise au sol de l'abri :

{{< alert icon="info" >}}
**Récapitulatif des surfaces (France) :**
* **Moins de 5 m² :** Aucune autorisation nécessaire.
* **De 5 à 20 m² :** Déclaration préalable en mairie obligatoire.
* **Plus de 20 m² :** Permis de construire requis.
{{< /alert >}}

![Martin remplit une déclaration administrative avec Anna à ses côtés](declaration-urbanisme.webp)
...
# Écriture du fichier avec encodage UTF-8 sans BOM
[System.IO.File]::WriteAllLines("$DirPath/index.md", $Content, $Utf8NoBom)

Write-Host "✅ Titre mis à jour : Guide 2026 activé !" -ForegroundColor Green